 ...
 */
int apultra_build_suffix_array(apultra_matchfinder *pCompressor, const unsigned char *pInWindow, const int nInWindowSize) {
 ...
 */
int apultra_find_matches_at(apultra_matchfinder *pCompressor, const int nOffset, apultra_match *pMatches, unsigned short *pMatchDepth, unsigned char *pMatch1, const int nMaxMatches, const int nBlockFlags) {
   unsigned long long *intervals = pCompressor->intervals;
   unsigned long long *pos_data = pCompressor->pos_data;
 ...
 * @param nBlockFlags bit 0: 1 for first block, 0 otherwise; bit 1: 1 for last block, 0 otherwise
 */
void apultra_find_all_matches(apultra_matchfinder *pCompressor, const int nMatchesPerOffset, const int nStartOffset, const int nEndOffset, const int nBlockFlags) {
 ...
 * @return 0 for success, non-zero for failure
 */
int apultra_matchfinder_init(apultra_matchfinder *pMatchfinder, const int nBlockSize, const int nMaxWindowSize, const int nMatchedPerIndex) {
   int nResult;
   ...
   if (!nResult) {
      ...
      if (pMatchfinder->intervals) {
         ...
         if (pMatchfinder->pos_data) {
            pMatchfinder->open_intervals = (unsigned long long *)malloc((LCP_AND_TAG_MAX + 1) * sizeof(unsigned long long));
            if (pMatchfinder->open_intervals) {
               pMatchfinder->match = (apultra_match *)malloc(nBlockSize * nMatchedPerIndex * sizeof(apultra_match));
               if (pMatchfinder->match) {
                  pMatchfinder->match_depth = (unsigned short *)malloc(nBlockSize * nMatchedPerIndex * sizeof(unsigned short));
                  if (pMatchfinder->match_depth) {
